      ...
      <div className="space-y-2">
        <div className="flex items-center space-x-2">
          ...
          <div className="inline-flex items-center">
            ...
            {field.type === 'integer' && (
              <span className="px-2 py-1 text-xs rounded bg-blue-100 text-blue-800 font-medium">integer</span>
            )}
            {field.type === 'number' && (
              <span className="px-2 py-1 text-xs rounded bg-green-100 text-green-800 font-medium">number</span>
            )}
            {(field.type === 'string' || field.type === 'text') && (
              <span className="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-800 font-medium">string</span>
            )}
            {field.type === 'select' && (
              <span className="px-2 py-1 text-xs rounded bg-indigo-100 text-indigo-800 font-medium">select</span>
            )}
            {field.type === 'checkbox' && (
              <span className="px-2 py-1 text-xs rounded bg-purple-100 text-purple-800 font-medium">boolean</span>
            )}
